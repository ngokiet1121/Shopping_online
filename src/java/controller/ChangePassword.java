...
public class ChangePassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String new_passMD5 = helper.MD5Generator.generate(new_password);
        if (passMD5.equals(user.getPassword())) {
            if (new_password.equals(confirm_password)) {
                if (userDao.find(user.getEmail()) != null) {
                    user.setPassword(new_passMD5);
                    Controller.controller.changePassWord(user);
                    helper.SendMailTLS.sendEmail(user.getEmail(), "Your new Password", "Mat khau moi cua ban la :" + new_password);
